 ...
 * @version 1.0.0
 */
public abstract class FirestormTransaction extends FirestormOperation
    ...
    public final <T> T get(final Class<T> objectClass, final String documentID) throws TransactionException {
        try {
            final DocumentReference documentReference = FS.firestore.collection(objectClass.getSimpleName()).document(documentID);
    ...
    public final <T> List<T> list(final Class<T> objectClass, final int limit) throws TransactionException {
        try {
            FS.checkRegistration(objectClass);
            ApiFuture<QuerySnapshot> future = FS.firestore.collection(objectClass.getSimpleName()).limit(limit).get();
    ...
    public final <T> ArrayList<T> list(final Class<T> objectClass) throws TransactionException {
        try {
            FS.checkRegistration(objectClass);
            ApiFuture<QuerySnapshot> future = FS.firestore.collection(objectClass.getSimpleName()).get();
            List<QueryDocumentSnapshot> documents = future.get().getDocuments();
            final int NUM_OF_DOCUMENTS = documents.size();
            DocumentReference[] documentReferences = new DocumentReference[NUM_OF_DOCUMENTS];
            for (int i = 0; i < documents.size(); i++) {
    ...
    public final <T> TransactionFilterable<T> filter(final Class<T> objectClass) {
        return new TransactionFilterable<>(FS.firestore.collection(objectClass.getSimpleName()), objectClass, transaction);
    }
